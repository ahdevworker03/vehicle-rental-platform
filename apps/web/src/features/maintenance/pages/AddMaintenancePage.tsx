...
export default function AddMaintenancePage() {
  ...
  return (
    <div className="min-h-full">
      ...
      <div className="mx-auto w-full max-w-5xl space-y-4 px-4 pb-24 pt-4 sm:px-6 lg:space-y-5">
        ...
        <FormSection title="المركبة" description="اختر المركبة المرتبط بها سجل الصيانة." contentClassName="md:grid-cols-1">
          <FormField label="المركبة" required error={errors.vehicle} htmlFor="maintenance-vehicle-search">
            <button type="button" onClick={() => setShowVehiclePicker((current) => !current)} aria-expanded={showVehiclePicker} aria-controls="maintenance-vehicle-options" className="flex min-h-11 w-full items-center justify-between gap-3 rounded-lg border border-input bg-card px-3 py-2.5 text-start shadow-sm transition-colors hover:bg-muted/35 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30"><span className="flex min-w-0 items-center gap-2"><span className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary"><Car className="size-4" aria-hidden="true" /></span><span className="min-w-0">{selectedVehicle ? <><span dir="ltr" className="block truncate text-sm font-semibold text-foreground">{selectedVehicle.make} {selectedVehicle.model}</span><span dir="ltr" className="number-ltr mt-0.5 block text-xs text-muted-foreground">{selectedVehicle.plateNumber}</span></> : <span className="text-sm text-muted-foreground">اختر مركبة</span>}</span></span><span className="shrink-0 text-xs font-medium text-primary">{showVehiclePicker ? "إخفاء" : "اختيار"}</span></button>
          </FormField>
          {showVehiclePicker && <div id="maintenance-vehicle-options" className="rounded-lg border border-border bg-card p-3 shadow-sm"><div className="relative"><Search className="pointer-events-none absolute inset-y-0 start-3 my-auto size-4 text-muted-foreground" aria-hidden="true" /><input id="maintenance-vehicle-search" type="search" value={vehicleSearch} onChange={(event) => setVehicleSearch(event.target.value)} placeholder="ابحث برقم اللوحة أو باسم المركبة..." className={`${inputClass} ps-10`} /></div><div className="mt-3 max-h-60 space-y-1 overflow-y-auto pe-1">{vehiclesQuery.isLoading ? <LoadingState rows={2} className="p-0" /> : filteredVehicles.length === 0 ? <EmptyState icon={Car} title="لا توجد مركبات مطابقة" description="جرّب كلمة بحث مختلفة." className="py-6" /> : filteredVehicles.map((vehicle) => <button key={vehicle.id} type="button" onClick={() => { setSelectedVehicleId(vehicle.id); setShowVehiclePicker(false); clearError("vehicle"); }} className={`flex w-full items-center justify-between gap-3 rounded-lg border px-3 py-2.5 text-start transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40 ${selectedVehicleId === vehicle.id ? "border-primary bg-primary/5 text-primary" : "border-transparent hover:border-border hover:bg-muted/45"}`}><span className="min-w-0"><span dir="ltr" className="block truncate text-sm font-semibold text-foreground">{vehicle.make} {vehicle.model}</span><span dir="ltr" className="number-ltr mt-0.5 block text-xs text-muted-foreground">{vehicle.plateNumber}</span></span>{selectedVehicleId === vehicle.id && <CheckCircle2 className="size-4 shrink-0 text-primary" aria-label="محددة" />}</button>)}</div></div>}
        </FormSection>
