          ...
          <TableBody>
            {customers.map((customer) => (
              ...
              >
                ...
                <TableCell className="min-w-[10rem] text-end">
                  <IdentityValue value={customer.phone} />
                </TableCell>
                <TableCell className="min-w-[12rem] text-end">
                  <IdentityValue value={customer.nationalId} />
                </TableCell>
                <TableCell className="min-w-[12rem] text-end">
                  <IdentityValue value={customer.licenseNumber} />
                </TableCell>
        ...
        {customers.map((customer) => (
          <article key={customer.id} className="space-y-4 px-4 py-4">
            ...
            <div className="grid grid-cols-2 gap-3 rounded-lg bg-muted/45 p-3">
              <div>
                <div className="ui-label">الهوية</div>
                <div className="mt-1">
                  <IdentityValue value={customer.nationalId} />
                </div>
              </div>
              <div>
                <div className="ui-label">الرخصة</div>
                <div className="mt-1">
                  <IdentityValue value={customer.licenseNumber} />
                </div>
              </div>
  ...
  return (
    <div aria-label="جارٍ تحميل العملاء" className="space-y-0">
      {[0, 1, 2, 3].map((row) => (
        <div
          key={row}
          className="grid grid-cols-4 gap-5 border-b border-border px-5 py-5 last:border-b-0"
        >
