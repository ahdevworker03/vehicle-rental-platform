...
import { Banknote, CheckCircle2, CircleDollarSign, Loader2, Plus, Wallet } from "lucide-react";
...
import { usePaymentMutations, useRentalPayments } from "@/features/payments/hooks";
...
export function PaymentSection({ rentalId, totalAmount }: PaymentSectionProps) {
  ...
  if (payments.isLoading) {
    return (
      <SectionCard title="الفوترة والمدفوعات" description="إجمالي العقد، المدفوع، والرصيد المتبقي.">
        <LoadingState rows={3} />
      </SectionCard>
    );
  }
    ...
    return (
      <SectionCard title="الفوترة والمدفوعات">
        <ErrorState
          ...
          description={payments.error ? getApiErrorMessage(payments.error).title : "تعذر تحميل المدفوعات. حاول مرة أخرى."}
  ...
  const paidAmount = paymentList.reduce((total, payment) => total + payment.amount, 0);
  const paymentStatus =
    ...
    >
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3 border-b border-border pb-5 sm:grid-cols-4">
          {totalAmount !== undefined && <FinanceValue label="إجمالي العقد" value={formatCurrency(totalAmount)} />}
          <FinanceValue label="المدفوع" value={formatCurrency(paidAmount)} tone="positive" />
          <FinanceValue label="الرصيد المتبقي" value={formatCurrency(outstandingBalance)} tone={outstandingBalance > 0 ? "warning" : "positive"} />
          <div className="space-y-2">
            <span className="ui-label">حالة الدفع</span>
        ...
        {showForm && isOwner && (
          <div className="space-y-4 rounded-lg border border-border bg-muted/35 p-4">
            ...
            <div className="flex flex-wrap gap-2">
              <Button type="button" variant="outline" onClick={() => setShowForm(false)} disabled={mutations.create.isPending}>
                إلغاء
              </Button>
              <Button type="button" onClick={handleSubmit} disabled={mutations.create.isPending}>
                {mutations.create.isPending ? <Loader2 className="size-4 animate-spin" aria-hidden="true" /> : <Banknote className="size-4" aria-hidden="true" />}
                تسجيل الدفعة
              </Button>
            </div>
          </div>
          ...
          {paymentList.length === 0 ? (
            ...
            <div className="mt-3 divide-y divide-border rounded-lg border border-border">
              {paymentList.map((payment) => (
                <div key={payment.id} className="grid grid-cols-[minmax(0,1fr)_auto] gap-3 px-3 py-3 sm:grid-cols-3 sm:items-center">
                  <div className="min-w-0">
                    <div className="text-xs text-muted-foreground">تاريخ الدفع</div>
                    <div className="number-ltr mt-1 text-sm font-medium text-foreground">{formatDateTime(payment.paymentDate)}</div>
                  </div>
                  <div className="hidden sm:block">
                    <div className="text-xs text-muted-foreground">الطريقة</div>
                    <div className="mt-1 text-sm font-medium text-foreground">{PAYMENT_METHOD_LABELS[payment.method] ?? payment.method}</div>
                  </div>
                  <div className="text-end">
                    <div className="text-xs text-muted-foreground sm:hidden">{PAYMENT_METHOD_LABELS[payment.method] ?? payment.method}</div>
                    <div className="number-ltr mt-1 text-sm font-bold text-foreground">{formatCurrency(payment.amount)}</div>
                  </div>
                </div>
              ))}
            </div>
...
function FinanceValue({ label, value, tone = "default" }: { label: string; value: string; tone?: "default" | "positive" | "warning" }) {
  return (
    <div className="min-w-0 space-y-2">
      <span className="ui-label">{label}</span>
      <div className={`number-ltr truncate text-base font-bold ${tone === "positive" ? "text-status-positive" : tone === "warning" ? "text-status-warning" : "text-foreground"}`}>
        {value}
      </div>
    </div>
  );
