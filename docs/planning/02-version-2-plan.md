...
# Task recurrence migration — atomic deployment required

The Task recurrence migration replaces and removes the legacy
recurrence_type column.

Deployment must not use a rolling old/new API deployment across this
migration.

Required sequence:

1. Stop/drain old API instances.
2. Apply database migration.
3. Start the new API version.
4. Verify Task API health and recurrence operations.
5. Deploy/enable the compatible frontend.

A short maintenance window is acceptable for this migration.
