...
| ---- | ---------------------- | --------------------------------------------------------------------------------------------------- |
| 57   | Milestone 5.8 complete | Establishes the release truth, scope, providers, operation matrix, and measurable gates.            |
...
### Current State
- Milestone 5.8 is closed. Architecture documents establish the component topology and PostgreSQL authority but not concrete providers, synchronization contracts, capacity targets, or recovery objectives.
...
- Approve the smallest production topology using the existing React, Express, PostgreSQL, Prisma, object-storage, and email boundaries.
...
- Select an email adapter/provider and define reset-link origin, sender identity, configuration, failure, and observability requirements; select private durable object storage through the existing provider abstraction.
- Define representative pilot data volumes, devices, networks, browser support, and API/frontend/search/sync performance targets; define staging/production environments, release ownership, backup retention, RPO/RTO, restore verification, and go/no-go authority.
...
- Production providers and environment boundaries are selected without introducing unneeded services; pilot capacity/performance targets and release-test data are documented and reproducible.
...
### Objective
Create reproducible staging/production deployment and operating procedures for the frontend, API, PostgreSQL, object storage, email, monitoring, backups, migrations, rollback, and smoke verification.
...
- Configure frontend static hosting, SPA rewrites, cache rules for hashed assets/HTML/service worker, security headers, API origin, and HTTPS domains.
- Provision separate staging and production PostgreSQL, private object storage, email, secrets, logging/error monitoring, and uptime/readiness monitoring.
...
- Define automatic database backups, object-storage durability/retention, monitoring, access control, and a timed restore drill verifying business/file consistency.
